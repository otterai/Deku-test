...
class Rohit:
    ...
    def __init__(self, DB_URL, DB_NAME):
        ...
        self.extra_fsub = self.database['extra_fsub']  # Initialize extra_fsub collection
    ...
    async def add_extra_fsub_link(self, link):
        """
        Add a new link to the extra_fsub collection.
        :param link: The link to be added.
        """
        try:
            await self.extra_fsub.insert_one({"link": link})
        except Exception as e:
            logging.error(f"Failed to add extra fsub link: {e}")

    async def get_all_extra_fsub_links(self):
        """
        Retrieve all links from the extra_fsub collection.
        :return: A list of links.
        """
        try:
            return [x async for x in self.extra_fsub.find()]
        except Exception as e:
            logging.error(f"Failed to retrieve extra fsub links: {e}")
            return []

    async def remove_all_extra_fsub_links(self):
        """
        Remove all links from the extra_fsub collection.
        """
        try:
            await self.extra_fsub.de
